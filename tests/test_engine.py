...
def test_all_requested_types_roundtrip():
    text = "Jane Smith works at International Business Machines (IBM) with GinnieMae / GNMA. Host prod-server.example.com: 10.2.3.4. SSN 123-45-6789. Card 4111 1111 1111 1111."
    candidates = detect(text, [])
    by_value = {c.original: c.kind for c in candidates}
    for value in ["Jane Smith", "International Business Machines", "IBM", "GinnieMae", "GNMA", "prod-server.example.com", "10.2.3.4", "123-45-6789", "4111 1111 1111 1111"]:
...
def test_simultaneous_longest_and_boundaries():
    output, counts = replace_exact("IBM IBMx IBM Corp IBM", {"IBM": "ABC", "IBM Corp": "XYZ", "ABC": "NO"})
    assert output == "ABC IBMx XYZ ABC"
    assert counts == {"IBM": 2, "IBM Corp": 1}


def test_manual_multiline_unicode_exact():
    text = "😀 Confidential\n東京の研究 — José Álvarez"
    candidates = [Candidate("東京の研究", "Custom", "REDACTED-R1234ABCD", "manual"), Candidate("José Álvarez", "Personal name", "Leia Organa RABC12345", "manual")]
    mappings, conversions = prepare_mappings(candidates, [], text)
    assert restore(replace_exact(text, conversions)[0], mappings)[0] == text


def test_old_alias_retained_after_replacement_edit():
    candidates = [Candidate("Jane Smith", "Personal name", "Leia Organa R1234ABCD", "manual")]
    mappings, _ = prepare_mappings(candidates, [], "Jane Smith")
    candidates[0].replacement = "Luna Lovegood R5678ABCD"
    updated, _ = prepare_mappings(candidates, mappings, "Jane Smith")
    assert restore("Leia Organa R1234ABCD and Luna Lovegood R5678ABCD", updated)[0] == "Jane Smith and Jane Smith"
...
@pytest.mark.parametrize("replacement", ["", " Jane ", "Jane Smith", "Jane Smith alias", "x\ny"])
def test_unsafe_replacements_rejected(replacement):
    with pytest.raises(ValueError):
        prepare_mappings([Candidate("Jane Smith", "Personal name", replacement, "manual")], [], "Jane Smith")
...
def test_occurrence_replacement_rejects_conflicting_overlap():
    ...
    text = "IBM Corp and IBM"
    candidates = [Candidate("IBM Corp", "Business", "Acme R11111111", "test"),
                  Candidate("IBM", "Business", "Wonka R22222222", "test", True, {13})]
